    ...
    return (
        <div className="group relative bg-white rounded-xl border border-secondary-100 p-5 shadow-sm hover:shadow-2xl hover:shadow-brand-200/40 hover:border-brand-200 transition-all duration-300 flex flex-col h-full hover:-translate-y-1 overflow-hidden">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                        <Link to={`/jobs/${job.id}`} className="block">
                            <h3 className="text-lg font-bold text-brand-900 group-hover:text-brand-600 transition-colors line-clamp-1 mb-1" title={job.name}>
                                {job.name}
                            </h3>
                        </Link>
                        {job.hot && (
                            <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-brand-50 text-brand-700 border border-brand-100 ml-2">
                                HOT
                            </span>
                        )}
                    </div>
                    <Link to={`/companies/${job.company?.id}`} className="text-sm text-secondary-500 hover:text-brand-600 transition-colors line-clamp-1 flex items-center gap-1 font-medium">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
                <div className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold bg-brand-50 text-brand-700 border border-brand-100">
            ...
            <div className="mt-auto pt-4 border-t border-secondary-50 flex items-center justify-between text-xs text-secondary-400">
                <div className="flex items-center gap-1">
                    <CalendarDays className="w-3.5 h-3.5" />
                    {job.createdAt ? formatDistanceToNow(new Date(job.createdAt), { addSuffix: true, locale: vi }) : 'Mới đăng'}
                </div>
                <span className="px-2 py-1 rounded bg-secondary-50 text-secondary-500 font-medium">
                    {job.type || 'Full-time'}
                </span>
            </div>
